 ...
 *
 * <p>Frontend: {@code sendToJava('get_cli_models:opencode')} ‚Üí
...
public class CliModelsHandler extends BaseMessageHandler {
    ...
    /** Cap on captured stdout ‚Äî a model list is small; this stops memory exhaustion. */
    ...
    private void listModels(String provider) {
        try {
            if ("codebuddy".equals(provider)
                ...
                pushError(provider, "–Ë“™ π”√±æµÿ≈‰÷√–≈œ¢", "CODEBUDDY_LOCAL_CONFIG_REQUIRED");
                return;
            }
            ...
            if ("dsh".equals(provider)) {
                // DSH model catalog comes from the live host ‚Äî honor the
            ...
            // Process exited; the reader hits EOF promptly ‚Äî join for the final lines.
